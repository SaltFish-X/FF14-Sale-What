<template>
  <el-row :gutter="20">
    ...
    <el-col :span="12">
      <el-radio-group v-model="listKey" @change="getCheckList">
        <el-radio-button v-for="item in checkLists" :label="item.label" />
      </el-radio-group>
    </el-col>
  </el-row>
...
const check = () => {
  ...
  value.value.forEach((itemId) => {
    getSaleHistory(
      ...
      // const maxPrice =
      //   saleList?.reduce((a, b) => Math.max(a, b.pricePerUnit), 0) || 0;
      // const minPrice =
      //   saleList?.reduce((a, b) => Math.min(a, b.pricePerUnit), 999999999) || 0;
